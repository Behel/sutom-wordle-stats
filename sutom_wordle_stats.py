...
# SUTOM :
(GAME_NAME, ID_GAME, DEAD_STR) = (
    "SUTOM",
    str((datetime.date.today() - datetime.date(2022, 1, 8)).days),
    "-",
)
(HISTO, HISTO_MAX) = ("🟦", "🟥")
...
def get_df_tweets():
    ...
    for tweet in tweepy.Paginator(
        client_get.search_recent_tweets,
        query='"' + GAME_NAME + " #" + ID_GAME + '"',
        start_time=BEGIN_TWEET_SCRAP.strftime("%Y-%m-%dT%H:%M:%SZ"),
...
def get_stats(tweets_df):
    ...
    zero = len(
        tweets_df.loc[
            tweets_df["tweet"].str.contains(
                GAME_NAME + " #" + ID_GAME + " " + DEAD_STR + "/6"
            )
            | tweets_df["tweet"].str.contains(
                GAME_NAME + " " + ID_GAME + " " + DEAD_STR + "/6"
            )
    ...
    smi_un = (
        HISTO_MAX * round(pct_un / 5)
        if un == max(zero, un, deux, trois, quatre, cinq, six)
        else HISTO * round(pct_un / 5)
    )
    smi_deux = (
        HISTO_MAX * round(pct_deux / 5)
        if deux == max(zero, un, deux, trois, quatre, cinq, six)
        else HISTO * round(pct_deux / 5)
    )
    smi_trois = (
        HISTO_MAX * round(pct_trois / 5)
        if trois == max(zero, un, deux, trois, quatre, cinq, six)
        else HISTO * round(pct_trois / 5)
    )
    smi_quatre = (
        HISTO_MAX * round(pct_quatre / 5)
        if quatre == max(zero, un, deux, trois, quatre, cinq, six)
        else HISTO * round(pct_quatre / 5)
    )
    smi_cinq = (
        HISTO_MAX * round(pct_cinq / 5)
        if cinq == max(zero, un, deux, trois, quatre, cinq, six)
        else HISTO * round(pct_cinq / 5)
    )
    smi_six = (
        HISTO_MAX * round(pct_six / 5)
        if six == max(zero, un, deux, trois, quatre, cinq, six)
        else HISTO * round(pct_six / 5)
    )
    smi_zero = (
        HISTO_MAX * round(pct_zero / 5)
        if zero == max(zero, un, deux, trois, quatre, cinq, six)
        else HISTO * round(pct_zero / 5)
    )
    ...
    tweet_to_send += (
        "2/6 - " + smi_deux + " " + str(deux) + " (Top " + str(top_deux) + "%)\n"
    )
    tweet_to_send += (
        "3/6 - " + smi_trois + " " + str(trois) + " (Top " + str(top_trois) + "%)\n"
    )
    tweet_to_send += (
        "4/6 - " + smi_quatre + " " + str(quatre) + " (Top " + str(top_quatre) + "%)\n"
    )
    tweet_to_send += (
        "5/6 - " + smi_cinq + " " + str(cinq) + " (Top " + str(top_cinq) + "%)\n"
    )
    tweet_to_send += (
        "6/6 - " + smi_six + " " + str(six) + " (Top " + str(top_six) + "%)\n"
    )
    ...
    tweet_to_send += (
        ...
        + str(
            round(
                (un + deux * 2 + trois * 3 + quatre * 4 + cinq * 5 + six * 6 + zero * 6)
                / total,
                2,
            )
        ).replace(".", ",")
